...
| 架构师 | 输出技术设计方案，制订统一 Code Standard | 技术栈选型、模块划分、接口定义、代码规范 |
| 开发者 | 根据需求/设计/规范生成多文件项目 | `### FILE:` 格式的完整项目代码 |
| 编译验证 | 真实编译/语法检查（非 LLM 模拟） | py_compile / node --check / mvn compile / go build |
| 代码审查 | LLM 审查代码质量、安全、需求符合性 | 审查报告 + pass/fail |
